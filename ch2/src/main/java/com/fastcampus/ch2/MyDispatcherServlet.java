...
// MethodCall3.java를 원격 프로그램으로 만든 예제
@WebServlet("/myDispatcherServlet")  //@WebServlet = @Controller + @RequestMapping
public class MyDispatcherServlet extends HttpServlet { // http://localhost/ch2/myDispatcherServlet?year=2021&month=10&day=1
	@Override
		...
		try {
			...
			for(int i=0;i<paramArr.length;i++) {
				...
				if(paramType==Model.class) {
					...
					String strValue = ((String[])value)[0];	// getParameterMap()에서 꺼낸 value는 String배열이므로 변환 필요. ?year=2021&year=2022 와 같이 value가 String[]로 들어올 수 있기 때문에
